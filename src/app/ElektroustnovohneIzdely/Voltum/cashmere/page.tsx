...
export default function VoltumCashmerePage() {
  ...
  const filteredProducts = useMemo(() => {
    return filterProducts(allProducts, (product: any) => {
      ...
      // Быстрая проверка на кашемир
      const hasCashmere = category.includes('кашемир') || 
      ...
      if (isMatch) {
        console.log(`[Voltum Cashmere] НАЙДЕН: "${product.name}"`);
      }
  ...
  useEffect(() => {
    ...
    if (filteredProducts.length > 0) {
      ...
      console.log('❌ Товары Voltum Cashmere не найдены');
    }
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Кашемир</h2>
