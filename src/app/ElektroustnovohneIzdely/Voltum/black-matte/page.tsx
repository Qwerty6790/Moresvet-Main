...
export default function VoltumBlackMattePage() {
  ...
  const filteredProducts = useMemo(() => {
    return filterProducts(allProducts, (product: any) => {
      ...
      // Быстрая проверка на черный матовый
      const hasBlackMatte = category.includes('черный матовый') || 
      ...
      const isMatch = hasBlackMatte && notChrome;
      
      // Логируем только подходящие товары
      if (isMatch) {
        console.log(`[Voltum Black Matte] НАЙДЕН: "${product.name}"`);
      }
  ...
  useEffect(() => {
    ...
    // Логируем результаты
    console.log(`✅ Найдено товаров Voltum Black Matte: ${filteredProducts.length}`);
    if (filteredProducts.length > 0) {
      ...
      console.log('❌ Товары Voltum Black Matte не найдены');
    }
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Черный матовый</h2>
