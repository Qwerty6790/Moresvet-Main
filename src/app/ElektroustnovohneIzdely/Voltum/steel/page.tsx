...
export default function VoltumSteelPage() {
  ...
  const filteredProducts = useMemo(() => {
    return filterProducts(allProducts, (product: any) => {
      ...
      // Быстрая проверка на сталь
      const hasSteel = category.includes('сталь') || 
                      name.includes('сталь') ||
                      name.includes('steel');
      ...
      const isMatch = hasSteel && notChrome;
      
      // Логируем только подходящие товары
      if (isMatch) {
        console.log(`[Voltum Steel] НАЙДЕН: "${product.name}"`);
      }
  ...
  useEffect(() => {
    ...
    // Логируем результаты
    console.log(`✅ Найдено товаров Voltum Steel: ${filteredProducts.length}`);
    if (filteredProducts.length > 0) {
      ...
      console.log('❌ Товары Voltum Steel не найдены');
    }
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Сталь</h2>
          ...
          <span>/</span>
          <span className="text-white">Сталь</span>
        </nav>
        </div>
