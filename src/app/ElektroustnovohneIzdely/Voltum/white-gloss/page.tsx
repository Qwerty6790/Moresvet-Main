...
export default function VoltumWhiteGlossPage() {
  ...
  const filteredProducts = useMemo(() => {
    return filterProducts(allProducts, (product: any) => {
      ...
      // Быстрая проверка на белый глянцевый
      const hasWhiteGloss = category.includes('белый глянцевый') || 
                           name.includes('белый глянцевый') ||
                           (name.includes('белый') && name.includes('глянцевый')) ||
                           (name.includes('white') && name.includes('gloss'));
      
      // Исключаем хром
      const notChrome = !category.includes('хром') && !name.includes('хром');
      
      const isMatch = hasWhiteGloss && notChrome;
      
      // Логируем только подходящие товары
      if (isMatch) {
        console.log(`[Voltum White Gloss] НАЙДЕН: "${product.name}"`);
      }
  ...
  const fetchProducts = async () => {
    try {
      ...
      const products = await fetchProductsOptimized('Voltum', { 
        ...
        include_name: 'белый,глянцевый,white,gloss'
      });
  ...
  useEffect(() => {
    ...
    // Логируем результаты
    console.log(`✅ Найдено товаров Voltum White Gloss: ${filteredProducts.length}`);
    if (filteredProducts.length > 0) {
      ...
      console.log('❌ Товары Voltum White Gloss не найдены');
    }
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Белый глянцевый</h2>
