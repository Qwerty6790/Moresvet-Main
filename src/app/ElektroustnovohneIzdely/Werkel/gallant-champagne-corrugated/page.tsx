...
export default function GallantChampagneCorrugatedPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Gallant и цвет шампань рифленый
          if (!name.includes('gallant')) return false;
          if (!name.includes('шампань') || !name.includes('рифленый')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Gallant - Шампань рифленый</h2>
          ...
          <span className="text-white">Gallant - Шампань рифленый</span>
        </nav>
        </div>
        </div>
