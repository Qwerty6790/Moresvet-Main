...
export default function VintageMokkoChromePage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Vintage и цвет мокко матовый хром
          if (!name.includes('vintage')) return false;
          if (!name.includes('мокко') || !name.includes('матовый') || !name.includes('хром')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto max-w-[1550px] px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Vintage - Мокко матовый хром</h2>
          ...
          <span>/</span>
          <span className="text-white">Vintage - Мокко матовый хром</span>
        </nav>
        </div>
