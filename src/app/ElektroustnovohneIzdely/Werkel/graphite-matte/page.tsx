...
export default function GraphiteMattePage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на '64'
          if (!article.endsWith('64')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Графит матовый</h2>
          ...
          <span className="text-white">Графит матовый</span>
        </nav>
        </div>
        </div>
