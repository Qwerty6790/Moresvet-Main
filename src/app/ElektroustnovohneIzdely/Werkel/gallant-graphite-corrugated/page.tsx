...
export default function GallantGraphiteCorrugatedPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Gallant и цвет графит рифленый
          if (!name.includes('gallant')) return false;
          if (!name.includes('графит') || !name.includes('рифленый')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Gallant - Графит рифленый</h2>
          ...
          <span className="text-white">Gallant - Графит рифленый</span>
        </nav>
        </div>
        </div>
