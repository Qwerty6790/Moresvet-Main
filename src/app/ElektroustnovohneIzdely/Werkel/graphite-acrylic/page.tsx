...
export default function GraphiteAcrylicPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на '44'
          if (!article.endsWith('44')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Графит акрил</h2>
          ...
          <span className="text-white">Графит акрил</span>
        </nav>
        </div>
        </div>
