...
export default function GallantWhitePage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Gallant и цвет белый
          if (!name.includes('gallant')) return false;
          if (!name.includes('белый')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto max-w-[1550px] px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-black mb-5">Gallant - Белый</h2>
          ...
          <span>/</span>
          <span className="text-black">Gallant - Белый</span>
        </nav>
        </div>
