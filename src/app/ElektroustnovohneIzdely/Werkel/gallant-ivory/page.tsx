...
export default function GallantIvoryPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Gallant и цвет слоновая кость
          if (!name.includes('gallant')) return false;
          if (!name.includes('слоновая') || !name.includes('кость')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Gallant - Слоновая кость</h2>
          ...
          <span className="text-white">Gallant - Слоновая кость</span>
        </nav>
        </div>
        </div>
