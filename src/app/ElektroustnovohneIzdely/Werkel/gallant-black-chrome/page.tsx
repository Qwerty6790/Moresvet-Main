...
export default function GallantBlackChromePage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Gallant и цвет черный хром
          if (!name.includes('gallant')) return false;
          if (!name.includes('черный') || !name.includes('хром')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Gallant - Черный хром</h2>
          ...
          <span className="text-white">Gallant - Черный хром</span>
        </nav>
        </div>
        </div>
