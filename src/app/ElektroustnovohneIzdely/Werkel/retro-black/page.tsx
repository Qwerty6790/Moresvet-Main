...
export default function RetroMetallicPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Ретро и металлическое
          if (!article.includes('08')) return false;
          if (!article.includes('08')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Retro - Черный</h2>
          ...
          <span className="text-white">Retro - Черный</span>
        </nav>
        </div>
        </div>
