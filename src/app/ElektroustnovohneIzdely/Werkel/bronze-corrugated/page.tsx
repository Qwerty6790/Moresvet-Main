...
export default function BronzeCorrugatedPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на '12'
          if (!article.endsWith('12')) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto max-w-[1550px] px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-black mb-5">Бронза рифленый</h2>
          ...
          <span>/</span>
          <span className="text-black">Бронза рифленый</span>
        </nav>
        </div>
