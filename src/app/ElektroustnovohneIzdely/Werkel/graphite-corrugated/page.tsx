...
export default function GraphiteCorrugatedPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на 'графит рифленый'
          if (!name.includes('графит рифленый')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Графит рифленый</h2>
          ...
          <span className="text-white">Графит рифленый</span>
        </nav>
        </div>
        </div>
