...
export default function WhiteAcrylicPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const params = {
        source: 'Werkel',
        page: 1,
        limit: 1000 // Загружаем все товары сразу
      };
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на '08'
          if (!article.endsWith('41')) return false;
          
          // Исключаем нежелательные категории (убираю пустую строку)
          const excludeTerms = ['vintage', 'ретро', 'retro', 'автоматический выключатель','дымчатый', 'gallant'];
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Белый акрил</h2>
          ...
          <span className="text-white">Белый акрил</span>
        </nav>
        </div>
        </div>
