...
export default function PearlescentCorrugatedPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на 'x4' или '13'
          if (!(article.endsWith('x4') || article.endsWith('13'))) return false;
  ...
  return (
    <div className="text-black" style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-black mb-5">Перламутровый рифленый</h2>
          ...
          <span className="text-white">Перламутровый рифленый</span>
        </nav>
        </div>
        </div>
