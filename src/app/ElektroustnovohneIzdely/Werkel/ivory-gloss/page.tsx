...
export default function IvoryGlossPage() {
    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем артикул заканчивается на '03'
          if (!article.endsWith('03')) return false;
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: '88rem' }}>
        <div className="mb-8">
          <h2 className="text-5xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-5">Слоновая кость глянцевый</h2>
          ...
          <span>/</span>
          <span className="text-white">Слоновая кость глянцевый</span>
        </nav>
        </div>
